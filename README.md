...
### v3.6.1 (2025-12-29)
- 🐛 **修复登录端点数据格式问题**
  - 修复 `/token` 端点只支持表单数据格式的问题
  - 现在同时支持 JSON 和表单数据两种格式
  - 修复了登录时出现 "Unexpected end of JSON input" 错误的问题

- 📝 **部署改进**
  - 添加 `.env.example` 环境配置模板文件
  - 添加自动化部署脚本（`deploy.sh` 和 `deploy.ps1`）
  - 添加详细的部署文档（`DEPLOYMENT.md`）
  - 添加部署检查清单（`DEPLOYMENT_CHECKLIST.md`）
  - 添加部署修复说明（`DEPLOYMENT_FIX.md`）
